...
integer ii
!Variables paral·lelització a parelles
...
allocate(pair(Natoms*(Natoms-1)/2,2))
allocate(pair_limits(numproc,2))

call assignacio(Natoms,numproc,index_particles,allgather_pointer,num_send,pair,pair_limits)
...
vel0,numproc,index_particles,num_send,allgather_pointer,taskid,pair,pair_limits)
