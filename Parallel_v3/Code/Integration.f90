...
! Last modification: 11/04/2021
	...
	INTEGER :: pair(N*(N-1)/2,2), limits(numproc,2)
	! ************************************************************************************************ !
    include 'mpif.h'
...
!    enddo
	...
	call force_LJ(N,L,rcut,rnew,numproc,index_part,Fnew,pot,pair,limits) ! New forces.	
...
!------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	SUBROUTINE Integrate(Nsteps,Npart,Nradial,T,dt,rho,rcut,L, &
     	sigma,thermostat,r0,v0,numproc,index_part,numsend,allgather,taskid,pair,limits)
	! ************************************************************************************************ !
	...
	INTEGER ierror
	! Parallel-pairs variables:
	INTEGER :: pair(Npart*(Npart-1)/2,2), limits(numproc,2)
	! ************************************************************************************************ !	
	include 'mpif.h'
...
2  	FORMAT(6(F14.8,2X))	
	...
	! Calculating forces
	call force_LJ(Npart,L,rcut,pos,numproc,index_part,forc,PE,pair,limits) 	
