...
integer, allocatable :: pair_limits(:,:)

! Lectura del fitxer amb les dades necessàries per la simulació
call read_files(numproc,temp,density,timestep,sigma,epsilon,mass,Natoms,Nsteps,Nradial)
...
allocate(pairs(Natoms*(Natoms-1)/2,2))
allocate(pair_limits(numproc,2))
...
call initial(Natoms,density,temp,taskid,pos0,vel0,L)
...
vel0,numproc,index_particles,num_send,allgather_pointer,taskid,pairs,pair_limits)
